...
public class FutariSocket
{
    ...
    public bool ConnectAsync(SocketAsyncEventArgs e, int mockID)
    {
        ...
        _client.tcpRecvQ[_streamId + _bindPort] = new ConcurrentQueue<FutariMsg>();
        _client.acceptCallbacks[_streamId + _bindPort] = msg =>
        {
            ...
            foreach (var handler in events?.GetInvocationList() ?? Array.Empty<Delegate>())
            {
        ...
        _client.sendQ.Enqueue(new FutariMsg
        {
            FutariCmd = FutariCmd.CTL_TCP_CONNECT, 
            proto = _proto,
            sid = _streamId,
    ...
    public FutariSocket Accept()
    {
        ...
        _client.tcpRecvQ[msg.sid.Value + _bindPort] = new ConcurrentQueue<FutariMsg>();
        _client.sendQ.Enqueue(new FutariMsg
        {
            FutariCmd = FutariCmd.CTL_TCP_ACCEPT, proto = _proto, sid = msg.sid,
            src = _client.StubIP.ToU32(), sPort = _bindPort,
    ...
    public int Send(byte[] buffer, int offset, int size, SocketFlags socketFlags)
    {
        ...
        _client.sendQ.Enqueue(new FutariMsg
        {
            FutariCmd = FutariCmd.DATA_SEND, proto = _proto, data = buffer.View(offset, size).B64(),
    ...
    public int SendTo(byte[] buffer, int offset, int size, SocketFlags socketFlags, EndPoint remoteEP)
    {
        ...
        _client.sendQ.Enqueue(new FutariMsg
        {
            FutariCmd = FutariCmd.DATA_BROADCAST, proto = _proto, data = buffer.View(offset, size).B64(), 
            src = _client.StubIP.ToU32(), sPort = _bindPort,
            dst = remote.Address.ToU32(), dPort = remote.Port
        });
        return size;
    ...
    public int ReceiveFrom(byte[] buffer, SocketFlags socketFlags, ref EndPoint remoteEP)
    {
        ...
        var data = msg.data?.B64() ?? Array.Empty<byte>();
    ...
    public void Close()
    {
        ...
        if (_proto == ProtocolType.Tcp) 
            _client.sendQ.Enqueue(new FutariMsg { FutariCmd = FutariCmd.CTL_TCP_CLOSE, proto = _proto });
    }
